#!/E:/Python/python.exe
# -*- coding: utf-8 -*-
#tukaj bo dodano shranjevanje - WIP
from Tkinter import *
...
global vse_igre
vse_igre = []
#shranjena_igra = open("igra.txt", "r+")
##print shranjena_igra.read()

#VALAT + BERAČ JE KONFLIKT
...
def glavna(col_sums):
	...
	#vse_igre.append(names)
	for i in rads:
		rads1.append(str(i.get()))
	for i in col_sums:
		col_sums1.append(str(i.get()))
	#vse_igre.append(names)
	#vse_igre.append(rads1)
	#vse_igre.append(col_sums1)
	vse_igre=[names,rads1,col_sums]
	...
	Button(c, text="Izhod", width=10, command = lambda: root.destroy()).grid(row=3, column=6, padx=10)
...
def write_to_players(players, summa):
	...
	if radl >=1: #če ima tisti, ki je igral radlc
		summa *= 2 #podvoji vsoto
		if zmaga == True: #če je zmagal
	...
	#print ("vse igre", vse_igre)
	
	"""for i in vse_igre[::-1]: profesorica pravi da je nesmiselno
		a=0
		for j in i:
			if a == 0:
				Label(c, width=10, borderwidth=4, text=j, font = (None, 10)).grid(row=rows, column=a)
			else:	
				Label(c, width=10, borderwidth=4, text=j, relief=SUNKEN).grid(row=rows, column=a)
			a+= 1
		rows += 1"""
...
#zadnja stvar, ki se zgodi
	root.update()
	f.config(scrollregion=f.bbox(x))


def shrani():
	with open("igra.txt", "a") as shranjena_igra:
		for i in vse_igre:
			for j in i:
				shranjena_igra.write(j)
				shranjena_igra.write(", ")
			shranjena_igra.write("\n")
...
def nastavitve():
	...
	i = 0
	l = []
	with open("igra.txt") as shranjena_igra:
			for line in shranjena_igra:
					print(line)
					l = line.split(", ")
					for j in l:
						if i != 6:
							if i == 0:
								Label(c, text=j, width=10, font=(None, 10)).grid(row=rows, column=i)
							else:
								Label(c, text=j, width=10, borderwidth=4, relief=SUNKEN).grid(row=rows, column=i)
							i += 1
					i = 0
					rows+=1
					l = []
